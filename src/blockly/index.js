...
export const load = (blockStr, dropEvent = {}) => {
    ...
    if (xml.hasAttribute('is_dbot')) {
        showDialog({
            ...
            text: [translate('Sorry, this strategy can’t be used with Binary Bot. You may only use it with Binarytool DBot.')],
            buttons: [
                {
                ...
                {
                    text: translate('Take me to Binarytool DBot'),
                    class: 'button-primary',
                    click() {
...
export default class _Blockly {
    ...
    // Custom Bot Loader
    loadCustomBot(botPath) {
        importFile(`public/bots/${botPath}`).then(dom => {
            Blockly.Events.setGroup('reset');
            Blockly.mainWorkspace.clear();
            Blockly.Xml.domToWorkspace(dom.getElementsByTagName('xml')[0], Blockly.mainWorkspace);
            Blockly.Events.setGroup(false);
