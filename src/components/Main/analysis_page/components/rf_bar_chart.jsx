...
function calculateRiseAndFallPercentages(numbers) {
    let riseCount = 0;
    let fallCount = 0;
    const totalComparisons = numbers.length - 1; // Total comparisons are one less than the number of elements
    ...
    const risePercentage = (riseCount / totalComparisons) * 100;
    const fallPercentage = (fallCount / totalComparisons) * 100;
...
export default class RiseFallBarChart extends PureComponent {
    render() {
        const { allDigitList, is_mobile } = this.props;
        const percentages = calculateRiseAndFallPercentages(allDigitList);
        const data = [
            {
                name: `Rise`,
